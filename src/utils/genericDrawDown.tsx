...
export const GenericDropDown = ({
  ...
  return (
    <div className={style.user_icon} ref={inputRef}>
      ...
      <label className={style.user_name_pic} htmlFor={type}></label>
      <div onClick={handleClick}>{dropDown}</div>
      <span ref={navRef} onClick={onNavClick} className={`${style.dropdown} ${dropDownMenuClass}`}>
        {dropDownMenu}
      </span>
    </div>
  );
...
export const createGenericDropDown = (type: string, dropDown?: JSX.Element, dropDownMenu?: JSX.Element, dropDownMenuClass?: any) => {
