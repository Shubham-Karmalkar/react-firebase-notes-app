  ...
  return (
    <div className={styles.content_wrapper} onClick={onClick}>
      <div className={styles.google_icon_wrapper}>
        <img className={styles.google_icon} alt="google icon" src="https://img.icons8.com/color/48/google-logo.png" />
      </div>
...
export const LogInWithGoogle = ({ callback }: { callback: any }) => {
  ...
  return <GoogleTemplate text="Log in With Google" onClick={onClickLogIn} />;
};
...
export const LogInWithCredentials = ({ email, password, callback }: { email: string; password: string; callback: any }) => {
  ...
  return (
    <div className={styles.log_in_btn} onClick={logIn}>
      <b>Log in</b>
    </div>
  );
};
...
export const SignUpWithCredentials = ({
  ...
  const signUp = async () => {
    if (password !== confPassword) return callback({ status: false, message: "Password did not Match" });
    try {
      const logInRes = await createUserWithEmailAndPassword(auth, email, password);
  ...
  return (
    <div className={styles.log_in_btn} onClick={signUp}>
      <b>Sign Up</b>
    </div>
  );
};
