...
const ColorArr = ["#87baf5", "#aa87f5", "#f0864a", "#f674ad", "#1f1c2f", "#8ac3a3"];
...
export const UserNotes = ({ setUpdateNote }: { setUpdateNote?: SetState<SimpleNote> }) => {
  ...
  const noteCards = !(filteredNotes && filteredNotes.length) ? (
    <div className={style.emptyNotes}>
      <img src={noDataImg} alt="no data present" />
    </div>
  ) : (
  ...
  return (
    <div className={style.userNotesContainer}>
      ...
      <div className={style.notesList}>{noteCards}</div>
    </div>
  );
};
