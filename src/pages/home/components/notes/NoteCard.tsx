...
const MoreOptions = ({ onDelete, setUpdateNote, onView }: { onDelete: any; setUpdateNote: any; onView: any }) => {
  return (
    <>
      <Link to="#" className={style.links} onClick={onView}>
        <LiaEye />
        <span>View</span>
      </Link>
      <Link to="#" className={style.links} onClick={setUpdateNote}>
        <SlPencil />
        <span>Edit</span>
      </Link>
      <Link to="#" className={style.links} onClick={onDelete}>
        <RiDeleteBin5Line />
        <span>Delete</span>
      </Link>
    </>
  ...
  return (
    <div className={`${style.noteCard} ${isView ? style.bigView : ""}`} style={{ borderBottom: `5px solid ${color}` }} ref={colorRef}>
      ...
      <div className={style.lowerCard}>
        <div>
          {userIcon}
          <span {...textColorClass}>{userText}</span>
        </div>
        <div>
          <BiCalendar {...colorClass} />
          <span {...textColorClass}>{localDate}</span>
        </div>
      </div>
    </div>
  );
